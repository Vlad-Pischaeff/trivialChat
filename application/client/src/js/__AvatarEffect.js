...
export default function __AvatarEffect(props) {
  const { refAvatar, refBg } = props
  const [ timerId, setTimerId ] = useState(null)

  useEffect(() => {
    if (refAvatar.current && refBg.current) {
      setTimerId(setInterval(() => {
        grad = grad.map((n, i) => n += idx[i])
        // console.log('__AvatarEffect..2..', refBg, refAvatar)
        val = val.map((n, i) => percent(grad[i]))
        refAvatar.current.style.borderRadius = `${val[0]}% ${100 - val[0]}% ${val[1]}% ${100 - val[1]}% 
                                  / ${val[2]}% ${val[3]}% ${100 - val[3]}% ${100 - val[2]}%`
  
        bg_grad = bg_grad.map((n, i) => n += bg_idx[i])
        bg_val = bg_val.map((n, i) => percent(bg_grad[i]))
        refBg.current.style.borderRadius = `${bg_val[0]}% ${100 - bg_val[0]}% ${bg_val[1]}% ${100 - bg_val[1]}% 
  ...
  }, [refAvatar.current, refBg.current])
}